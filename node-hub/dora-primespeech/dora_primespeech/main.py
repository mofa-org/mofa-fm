...
def main(node_id=None):
    ...
    Args:
        node_id: Optional node ID for dynamic nodes. If None, uses environment.
    """

    node = Node(node_id) if node_id else Node()
    ...
    try:
        print("[PRIMESPEECH] Starting pre-initialization...", flush=True)
        ...
        # Support both CUDA and MPS devices
        if config.USE_GPU:
            if config.DEVICE.startswith("cuda"):
                ...
                device = config.DEVICE  # Use whatever device is specified
        else:
            device = "cpu"
        enable_streaming = config.RETURN_FRAGMENT if hasattr(config, 'RETURN_FRAGMENT') else False

        print(f"[PRIMESPEECH] Creating wrapper: voice={moyoyo_voice}, device={device}", flush=True)
        ...
        print(f"[PRIMESPEECH] Wrapper created, verifying...", flush=True)

        # Verify initialization
        if tts_engine is None:
            print("[PRIMESPEECH] ERROR: tts_engine is None!", flush=True)
            raise RuntimeError("TTS engine is None")
        elif not hasattr(tts_engine, 'tts'):
            print("[PRIMESPEECH] ERROR: tts_engine has no 'tts' attribute!", flush=True)
            raise RuntimeError("TTS engine has no 'tts' attribute")
        elif tts_engine.tts is None:
            print("[PRIMESPEECH] ERROR: tts_engine.tts is None!", flush=True)
            raise RuntimeError("Internal TTS engine not initialized")
        else:
            print(f"[PRIMESPEECH] Wrapper verified OK, tts={type(tts_engine.tts)}", flush=True)
        ...
        print(f"[PRIMESPEECH] PRE-INIT FAILED: {init_err}", flush=True)
        print(f"[PRIMESPEECH] Traceback: {traceback.format_exc()}", flush=True)
    ...
    print(f"[PRIMESPEECH] Entering event loop, ready to receive text input...", flush=True)
    send_log(node, "INFO", "Entering event loop, ready to receive text input", config.LOG_LEVEL)

    for event in node:
        event_type = event["type"]
        send_log(node, "DEBUG", f"Received event type: {event_type}", config.LOG_LEVEL)

        if event_type == "INPUT":
            input_id = event["id"]
            send_log(node, "DEBUG", f"INPUT event id: {input_id}", config.LOG_LEVEL)
