...
class StreamingMoYoYoTTSWrapper:
    ...
    def _init_tts(self):
        ...
        try:
            ...
            print(f"[WRAPPER] TTS object created successfully", flush=True)
            ...
            print(f"[WRAPPER] Reference audio path: {self.ref_audio_path}", flush=True)

            self.log("INFO", f"Reference audio: {self.ref_audio_path}")
            if not Path(self.ref_audio_path).exists():
                print(f"[WRAPPER ERROR] Reference audio does not exist: {self.ref_audio_path}", flush=True)
                self.log("ERROR", f"Reference audio does not exist: {self.ref_audio_path}")

            # Pre-cache reference audio
            print(f"[WRAPPER] Setting reference audio...", flush=True)
            self.tts.set_ref_audio(self.ref_audio_path)
            print(f"[WRAPPER] Reference audio set successfully", flush=True)
            
            self.log("INFO", "MoYoYo TTS initialized successfully")
        except Exception as e:
            # Force flush error to stdout immediately
            print(f"[WRAPPER ERROR] Failed to initialize MoYoYo TTS: {e}", flush=True)
            import traceback
            print(f"[WRAPPER ERROR] Traceback:\n{traceback.format_exc()}", flush=True)
