...
def send_next_segment_for_participant(participant, node, log_level, segment_queues, is_sending, last_session_end_sent):
    ...
    # Filter out None values from metadata (dora doesn't accept None)
    metadata = {
        "session_id": segment["session_id"],
        "question_id": segment.get("question_id") or "unknown",
        "session_status": segment.get("session_status") or "unknown"
    }
...
def main():
    ...
    def kick_start_sending(participant):
        ...
        # Remove any None values that might still exist
        metadata = {k: v for k, v in metadata.items() if v is not None}
