...
impl ConferenceController {
    ...
    fn process_next_speaker(&mut self, node: &mut DoraNode) -> Result<()> {
        ...
        if let Some(next_speaker) = self.policy.determine_next_speaker() {
            ...
            // BUGFIX: Disable session_start waiting for text-only dataflows
            // Text-only dataflows (like debate-sequential) don't have audio player
            // so they never send session_start signals. Commenting this out allows
            // sequential speakers to proceed immediately without waiting.
            // TODO: Add environment variable to enable/disable this for audio dataflows
