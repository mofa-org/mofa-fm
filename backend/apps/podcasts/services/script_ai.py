...
class ScriptAIService:
    ...
    def _build_system_prompt(
        ...
        base_prompt = (
            ...
            "## 重要：工具使用规则\n"
            "**必须使用 tavily_search 工具的情况（满足任一条件即使用）：**\n"
            "1. 用户明确要求\"搜索\"、\"查询\"、\"查找\"等\n"
            "2. 涉及\"今天\"、\"最新\"、\"最近\"、\"当前\"等时间词\n"
            "3. 涉及\"热点\"、\"热门话题\"、\"新闻\"、\"动态\"等\n"
            "4. 需要具体数据：股价、指数、天气、比分等实时信息\n"
            "5. 用户提到具体事件名称（如\"特斯拉降价\"、\"AI新政策\"等）需要背景资料\n"
            "6. 询问\"发生了什么\"、\"有什么大事\"等开放性问题\n\n"
            "**不要凭空编造数据和新闻！遇到上述情况，必须先搜索，再基于真实结果回答。**\n\n"
            "## 脚本格式要求（仅在模式2时使用）\n"
            ...
            "6. **如果使用了搜索工具，必须基于搜索结果中的真实信息创作，不要编造**\n\n"
            "## 输出示例\n"
            "**示例 1: 正常对话模式**\n"
            "用户: 今天沪指怎么样？\n"
            "助手: 今天沪指收于3870.02点，上涨0.87%，成交量温和放大。主要是工程机械、医药板块带动，整体呈现温和反弹态势。\n\n"
            "**示例 2: 脚本创作模式**\n"
            "用户: 帮我写个播客讲今天的沪指行情\n"
            "助手:\n"
            "# 今天沪指：小幅上涨，成交温和\n\n"
