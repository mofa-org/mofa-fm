#!/bin/bash
# Chatbot Alicloud 0908 启动脚本（交互式配置）
set -Eeuo pipefail

echo "============================================"
echo "启动 Chatbot Alicloud 0908"
echo "============================================"
...
if [[ -z "${ALIBABA_CLOUD_API_KEY}" ]]; then
  echo "错误: ALIBABA_CLOUD_API_KEY 不能为空，终止。"
  exit 1
fi

echo "已设置 ALIBABA_CLOUD_API_KEY（长度: ${#ALIBABA_CLOUD_API_KEY}）"
